...
# Background
This post was created from a 2021 Pennsylvania State University (PSU) Stat 485 class project as a way for students to learn R and associated RStudio IDE.  I've copied the code as I created it during the class and re-hosted the app in a new location given that I am no longer a student.

This tutorial will review the source code which generates the Shiny App as demonstrated below.  This simply app provides a user interface for the user to provide a .csv dataset and the output will sort the R-squared correlation coefficient values so that the user can focus attention on the highly correlating values as a means for dimension reduction of "wide" datasets.

**WARNING: the app may not work if it has hit the monthly usage limit. Try again later if that is the case.**
...
1. A row with 3 columns
    ...
    3.	“action” button.  Once clicked, the analysis will filter based on the user’s input
...
<figcaption align = "center">Fig.6 - reactive source code for the "action" button</figcaption>
</p>
</figure>
...
*	“Action” is a button that the user can click to view the xy plot for that pair of variables 
...
After clicking any of the “Action” button for a given row ,the xy plot will be displayed below.  The code adds the Rsq to the main title based on the value from the selected row.
...
In closing, creating web apps with Shiny is a great way to provide end-users with a way to access and interact with their data without requiring them to install special software, or know how to code.  After this experience I am going to continue expanding on this project at work for connecting the data source to an SQL database, as well as investigate other options for hosting on our own servers for security and privacy reasons, as using the shiny.io platform would not be sufficient for an enterprise system.  After these pieces are in place, then the options are going to be nearly endless!  I am really excited at the possibility of unlocking the power of our existing data to the many users who are currently limited by the availability and expertise of programming and analysis tools.

Full source code can be found at [https://github.com/kljohnson4good/R-code/tree/main/Shiny-corr-coeff](https://github.com/kljohnson4good/R-code/tree/main/Shiny-corr-coeff)
